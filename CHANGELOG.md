...
### Fixed
- **Version display**: UI now reads version dynamically from `/health` endpoint instead of hardcoded string — always shows the actual running version.

## [1.4.12] - 2026-04-02

### Security / Fixed
- **Insecure default password**: First-run default admin account now uses `REMOVED-CREDENTIAL-ROTATED-20260828` instead of `admin`. Configurable via `DEFAULT_ADMIN_PASSWORD` env var.
...
### Fixed
- **Log viewer line limit**: Backend was hard-capping log reads at 2000 lines regardless of request. Cap raised to 100,000. UI dropdown now offers 500 / 1000 / 5000 / 20000 / All options instead of stopping at "All (2000)".
