...
**F-INFRA-003** — `tests/conftest.py:15` had `ADMIN_PASS = "Super*120120"` in plaintext (committed in git history on a public repo). Now reads from `LLMPROXY_TEST_ADMIN_PASS` env var with a dev-default fallback. Same pattern applied to `BASE_URL`/`ADMIN_USER`. The password remains in old commits — recommend operator rotate the live password if it's still in active use.
