...
async def lifespan(app: FastAPI):
    ...
    set_service_info(version="2.0.24", node_id=settings.cluster_node_id or "")
    init_tracer(service_name="llm-proxy", version="2.0.24")
...
app.add_middleware(
    ...
    expose_headers=[
        ...
        "X-Critique-Provider", "X-Cot-Samples",
    ],
)
...
@app.get("/health")
async def health():
    return {"status": "ok", "version": "2.0.24"}


@app.get("/version")
async def version():
    return {"service": "llm-proxy", "version": "2.0.24", "docs": "/docs"}
