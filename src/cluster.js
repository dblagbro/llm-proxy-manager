...
class ClusterManager extends EventEmitter {
  ...
  mergeConfiguration(remoteConfig, peerId) {
    ...
    // Merge providers (union + field sync, respecting tombstones)
    // Fields synced when remote is newer (by updatedAt): apiKey, type, baseUrl, model, name, priority
    // enabled=false always propagates; a peer can never re-enable a locally disabled provider
    const SYNCABLE_FIELDS = ['apiKey', 'type', 'baseUrl', 'model', 'name', 'priority'];
    if (remoteConfig.providers) {
      ...
      for (const provider of remoteConfig.providers) {
        ...
        if (!local) {
          ...
          // Propagate disabled state — can never re-enable via peer
          if (provider.enabled === false && local.enabled !== false) {
            local.enabled = false;
            changes++;
            this.logger.info(`Provider disabled via peer sync: ${provider.name}`);
          }
          // Propagate field updates when peer's record is newer
          const remoteTs = provider.updatedAt || 0;
          const localTs  = local.updatedAt || 0;
          if (remoteTs > localTs) {
            let updated = false;
            for (const field of SYNCABLE_FIELDS) {
              if (provider[field] !== undefined && provider[field] !== local[field]) {
                local[field] = provider[field];
                updated = true;
              }
            }
            if (updated) {
              local.updatedAt = remoteTs;
              changes++;
              this.logger.info(`Updated provider fields from peer: ${provider.name}`);
              // Persist to SQLite if available
              if (this.sqliteDb) {
                try { this.sqliteDb.saveProvider(local); } catch (_) {}
              }
            }
          }
        }
