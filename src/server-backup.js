...
// Translate Google Gemini response to Anthropic format
...
app.post('/v1/messages', async (req, res) => {
  ...
  logger.info('Received request', { model: req.body.model, messageCount: req.body.messages?.length });
...
app.get('/api/config', (req, res) => {
  // Don't send full API keys, just show if they're set
  const safeConfig = {
    ...config,
...
app.post('/api/config', (req, res) => {
  try {
    // Only update certain fields, not API keys
